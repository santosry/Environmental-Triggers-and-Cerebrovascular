...
⚠️  ATENÇÃO: AS TABELAS PRÉ-PROCESSADAS JÁ ESTÃO SALVAS EM data_processed/.
    Você NÃO precisa executar este script novamente para usar os dados.
    Os CSVs já estão prontos para uso imediato.

    Este script é OPCIONAL — use apenas se:
    - Precisar re-extrair os dados do Power BI (ex.: atualização do dashboard)
    - Quiser auditar o processo de extração de forma independente
    - Precisar modificar o downscaling temporal ou o perfil sazonal
...
Requisitos (apenas se for executar este script):
...
# ============================================================
BASE_DIR = Path(__file__).resolve().parent.parent  # 02_MP25_RJ_exposicao/
...
def extrair_perfil_mensal_nacional(responses: List[dict]) -> Dict[int, float]:
    ...
    for resp in responses:
        ...
        # Estrutura: [mes_idx, ano, pm25_val] (primeira linha) ou [mes_idx, pm25_val] (demais)
        ...
        break  # Só uma consulta de perfil mensal
    ...
    if not perfil_mensal or len(perfil_mensal) != 12:
        ...
        # Perfil sintético baseado em literatura de PM2.5 no Sudeste brasileiro
        # (pico em ago-out = estação seca + queimadas)
        perfil_sintetico = {
...
# ============================================================
# AGREGAÇÃO POR MACRORREGIÃO (MENSAL)
# ============================================================

def agregar_por_macroregiao_mensal(df_mensal: pd.DataFrame) -> pd.DataFrame:
    """
    Agrega PM25 MENSAL por macroregião (média dos municípios).
    Gera CSV compatível com o pipeline DLNM para controle de qualidade do ar.
    """
    logger.info("Agregando PM25 mensal por macroregião...")

    df_macro = (
        df_mensal
        .groupby(['Macroregiao', 'Ano', 'Mes'], as_index=False)
        .agg(
            PM25=('PM25', 'mean'),
    ...
    csv_macro = DATA_PROCESSED_DIR / "mp25_macroregiao_mensal_2010_2025.csv"
    df_macro.to_csv(csv_macro, index=False, encoding='utf-8', sep=';')
    logger.info(f"  CSV macroregião mensal: {csv_macro} ({len(df_macro)} registros)")
...
def salvar_resultados(
    ...
    cols_media = [
        'Municipio', 'Macroregiao', 'PM25_medio', 'Prop_dias',
    ...
    csv_stats = DATA_PROCESSED_DIR / "mp25_rj_estatisticas_macroregiao.csv"
    stats_macro.to_csv(csv_stats, index=False, encoding='utf-8', sep=';')
    logger.info(f"  CSV estatísticas: {csv_stats}")
...
def main():
    ...
    # Passo 7: Agregar por macroregião (mensal)
    df_macro = agregar_por_macroregiao_mensal(df_final)
    logger.info(f"Passo 7 OK: {len(df_macro)} registros macroregionais mensais")

    # Passo 8: Salvar tudo
    salvar_resultados(df_final, df_municipios, df_macro, perfil_mensal)
    logger.info("Passo 8 OK: resultados salvos")
