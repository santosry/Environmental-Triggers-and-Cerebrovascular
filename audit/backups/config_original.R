...
FORCE_RAW_DOWNLOAD <- tolower(
  ...
  c("1", "true", "sim", "yes")
# [S-017] SIM-SIH fallback control.
# When TRUE, missing SIM years are filled with SIH hospital deaths.
# When FALSE (default), missing SIM years are set to NA.
# FALSE is the recommended setting because SIM cause-of-death and SIH
# in-hospital death measure different epidemiological constructs.
SIM_SIH_FALLBACK <- tolower(
  Sys.getenv("DLNM_SIM_SIH_FALLBACK", unset = "false")) %in%
  c("1", "true", "sim", "yes")
